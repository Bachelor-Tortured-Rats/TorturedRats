...
class RandSelectPatchesd(RandomizableTransform, MapTransform):
    ...
    def __call__(self, data, randomize: bool = True):
        ...
        for key in self.key_iterator(d):
            ...
            # get random offset
            #randx = -np.random.randint(1,2+1) if self.rand[0] in [0,1,2,3,4,5,6,7,8] else np.random.randint(1,2+1)
            #randy = -np.random.randint(1,2+1) if self.rand[0] in [0,1,2,9,10,11,18,19,20] else np.random.randint(1,2+1)
            #randz = -np.random.randint(1,2+1) if self.rand[0] in [0,3,6,9,12,15,18,21,24] else np.random.randint(1,2+1)
