...
from src.data.hepatic_dataset import load_hepatic_dataset_aug, load_hepatic_dataset
...
def main(ircad_path,hepatic_path):
    IRCAD_train_loader, IRCAD_val_loader = load_hepatic_dataset(data_dir=hepatic_path,sample_size=2,aug=True)
    hepatic_train_loader, hepatic_val_loader = load_hepatic_dataset(data_dir=hepatic_path,sample_size=2,aug=False)

    IRCAD_val_sample = first(IRCAD_val_loader)
    hepatic_val_sample = first(hepatic_val_loader)
    
    fig, ax = plt.subplots(nrows=2, ncols=2, figsize=(15, 15)) 
    ax[0,0].set_title("IRCAD_train_sample, layer 66")
    ax[0,0].imshow(IRCAD_val_sample["image"][0][0][100:148, 100:148, 66], cmap="gray")
    ...
    ax[0,1].imshow(hepatic_val_sample["image"][0][0][100:148, 100:148, 66], cmap='gray')
    ...
    fig.savefig('reports/figures/visualization/HepaticAugDistribution.png')

    fig, ax = plt.subplots(nrows=2, ncols=2, figsize=(15, 15)) 
    ax[0,0].set_title("IRCAD_train_sample, layer 66")
    ax[0,0].imshow(IRCAD_val_sample["image"][0][0][:, :, 66], cmap="gray")
    ax[1,0].set_title("IRCAD_train_sample, historgram over intensity full image, excluding black pixels")
    counts, edges = np.histogram(IRCAD_val_sample["image"][0][0][:, :,:].reshape(-1), bins=30)
    ax[1,0].stairs(counts[1:], edges[1:], fill=True)
    ax[1,0].vlines(edges[1:], 0, counts[1:].max(), colors='w')
    ...
    fig.savefig('reports/figures/visualization/HepaticAugDistribution_full.png')
